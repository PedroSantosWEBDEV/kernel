...
use x86_64::structures::paging::{
    page::PageRangeInclusive, FrameAllocator, Mapper, OffsetPageTable, Page, PageTable, PhysFrame,
    Size4KiB, Translate,
};
...
// Memory map
pub(crate) static MEMMAP: Mutex<Option<&'static MemoryMap>> = Mutex::new(None);
...
pub fn init(bootinfo: &'static BootInfo) {
    interrupts::without_interrupts(|| {
        ...
        unsafe { PMEM_OFFSET = bootinfo.physical_memory_offset };

        let mut memmap = MEMMAP.lock();
        *memmap = Some(&bootinfo.memory_map);

        let mut mapper = unsafe { mapper(VirtAddr::new(PMEM_OFFSET)) };

        let mut framealloc = unsafe { BootInfoFrameAllocator::init(&bootinfo.memory_map) };
...
pub fn p_dealloc(address: u64, size: u64) {
    let mut mapper = unsafe { crate::mem::mapper(VirtAddr::new(crate::mem::PMEM_OFFSET)) };
...
pub fn ptov(address: PhysAddr) -> VirtAddr {
    VirtAddr::new(address.as_u64() + unsafe { PMEM_OFFSET })
}
...
impl BootInfoFrameAllocator {
    ...
    pub unsafe fn init(memmap: &'static MemoryMap) -> Self {
        BootInfoFrameAllocator { memmap, next: 0 }
    }
...
pub fn vtop(address: VirtAddr) -> Option<PhysAddr> {
    let mapper = unsafe { mapper(VirtAddr::new(PMEM_OFFSET)) };
